...
constexpr UINT kTrayToggleProcessingCommand = 1001;
constexpr UINT kTrayOpenSettingsCommand = 1002;
...
Win32Window::MessageHandler(HWND hwnd,
  ...
  if (message == kTaskbarCreatedMessage) {
    ...
    SetTaskbarProgress(taskbar_progress_state_, taskbar_progress_);
    return 0;
  }
  ...
  switch (message) {
    case kTrayIconMessage: {
      ...
      if (tray_event == NIN_SELECT || tray_event == NIN_KEYSELECT ||
          tray_event == WM_LBUTTONUP || tray_event == WM_RBUTTONUP ||
          tray_event == WM_CONTEXTMENU) {
        ShowTrayMenu();
      } else if (tray_event == WM_LBUTTONDBLCLK) {
        RestoreFromTray();
      }
      return 0;
    }
...
void Win32Window::SetTrayStatus(const std::wstring& tooltip,
                                bool task_action_enabled,
                                bool task_is_running) {
  tray_tooltip_ = tooltip.empty() ? L"Neri" : tooltip;
  tray_task_action_enabled_ = task_action_enabled;
  tray_task_is_running_ = task_is_running;
  ...
  tray_icon.cbSize = sizeof(NOTIFYICONDATAW);
  tray_icon.hWnd = window_handle_;
  tray_icon.uID = kTrayIconId;
  tray_icon.uFlags = NIF_TIP | NIF_SHOWTIP;
  wcsncpy_s(tray_icon.szTip, tray_tooltip_.c_str(), _TRUNCATE);
  Shell_NotifyIconW(NIM_MODIFY, &tray_icon);
}

void Win32Window::SetTaskbarProgress(TaskbarProgressState state,
                                     double progress) {
  taskbar_progress_state_ = state;
  taskbar_progress_ = std::clamp(progress, 0.0, 1.0);
  ...
  if (taskbar_list_ == nullptr) {
    if (FAILED(CoCreateInstance(CLSID_TaskbarList, nullptr,
        ...
        taskbar_list_ == nullptr || FAILED(taskbar_list_->HrInit())) {
      if (taskbar_list_ != nullptr) {
        taskbar_list_->Release();
        taskbar_list_ = nullptr;
      }
  ...
  taskbar_list_->SetProgressState(window_handle_, flag);
  if (flag == TBPF_NORMAL || flag == TBPF_PAUSED || flag == TBPF_ERROR) {
    constexpr ULONGLONG kProgressTotal = 10000;
    const ULONGLONG completed =
        static_cast<ULONGLONG>(taskbar_progress_ * kProgressTotal);
    taskbar_list_->SetProgressValue(window_handle_, completed, kProgressTotal);
  }
}
...
void Win32Window::ShowTrayMenu() {
  ...
  const wchar_t* task_label = tray_task_is_running_
                                  ? L"\u6682\u505c"
                                  : L"\u5f00\u59cb";
  AppendMenuW(menu,
              MF_STRING |
                  (tray_task_action_enabled_ ? MF_ENABLED : MF_GRAYED),
              kTrayToggleProcessingCommand, task_label);
  AppendMenuW(menu, MF_STRING, kTrayOpenSettingsCommand, L"\u8bbe\u7f6e");
  ...
  const UINT selected = TrackPopupMenuEx(
      ...
      cursor.x, cursor.y, window_handle_, nullptr);
  DestroyMenu(menu);
  PostMessage(window_handle_, WM_NULL, 0, 0);

  switch (selected) {
    case kTrayToggleProcessingCommand:
      OnTrayCommand(TrayCommand::kToggleProcessing);
      break;
    case kTrayOpenSettingsCommand:
      RestoreFromTray();
      OnTrayCommand(TrayCommand::kOpenSettings);
      break;
    case kTrayExitCommand:
      RestoreFromTray();
      OnTrayCommand(TrayCommand::kExit);
      break;
  }
}
...
void Win32Window::OnTrayCommand(TrayCommand command) {
  // No-op; provided for subclasses.
}
