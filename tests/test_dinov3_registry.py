...
def test_set_identity_keeps_same_named_candidates_addressable(tmp_path):
    reg=SpeciesRegistry(tmp_path/'r.db',model_fingerprint=FP)
    ...
    updated=reg.set_identity(second.id,common_name='骆驼')
    named=[entry for entry in reg.list() if entry.common_name=='骆驼']
    assert updated.id==second.id
    assert [entry.id for entry in named]==[first.id,second.id]
    assert [entry.event_count for entry in named]==[1,1]
    reg.close()