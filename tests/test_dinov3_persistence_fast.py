...
def test_fast_runtime_persistence_never_updates_existing_registry_entry(tmp_path):
    registry = _Registry()
    observations = [
    ...
    detector = SimpleNamespace(
        dinov3_feedback=None,
        dinov3_registry=registry,
        drain_dinov3_observations=lambda: tuple(observations),
    )
    paths = [tmp_path / 'accepted.jpg', tmp_path / 'rejected.jpg']
    ...
    assert registry.observations == []
    assert len(registry.unknowns) == 1
    assert registry.unknowns[0]['candidate_kind'] == 'candidate'
