...
from tests.dinov2_state_fixtures import make_multi_prototype_payload
...
def _write_model_bundle(tmp_path: Path) -> Path:
    ...
    manifest.write_text(
        '{"schema_version":2,"backend":"dinov2","checkpoint":"head.pt","architecture":"dinov2_vitb14","feature_dim":768}',
        encoding="utf-8",
    )
    return manifest
...
def _write_multi_prototype_bundle(tmp_path: Path) -> Path:
    ...
    manifest.write_text(
        '{"schema_version":2,"backend":"dinov2","checkpoint":"multi_prototype.pt","architecture":"dinov2_vitb14","feature_dim":768}',
        encoding="utf-8",
    )
    return manifest
...
def _client(monkeypatch, tmp_path):
    monkeypatch.setenv("NERI_DINOV2_STATE_DIR", str(tmp_path / "state"))
...
def _seed_registration(manifest: Path, *, event_count=5, set_identity=True):
    from system.backend.dinov2_registry_service import open_registry_for_model
...
def test_registry_list_detail_events_identity_and_register(monkeypatch, tmp_path):
    ...
    detail = client.get(
        f"/api/dinov2/registry/{registration_id}",
        params={"classification_model_path": str(manifest)},
    )
    ...
    events = client.get(
        f"/api/dinov2/registry/{registration_id}/events",
        params={"classification_model_path": str(manifest)},
    )
    ...
    identity = client.patch(
        f"/api/dinov2/registry/{registration_id}/identity",
        json={
    ...
    registered = client.post(
        f"/api/dinov2/registry/{registration_id}/register",
