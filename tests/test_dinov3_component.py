...
class _FakeDinoCloud:
    ...
    def download_file(self, relative_path: str, target: Path, on_progress=None) -> None:
        if self.fail_on == relative_path:
            raise RuntimeError("fake cloud failure")
        target.parent.mkdir(parents=True, exist_ok=True)
        if relative_path == "install.json":
            target.write_text(
                json.dumps(
                    {
                        "schema_version": 1,
...
class DinoV3ComponentLayoutTests(unittest.TestCase):
    ...
    def test_install_assembles_cloud_assets_and_seed_atomically(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp)
            root = base / "res" / "model" / "DINOv3"
            seed = base / "seed"
            seed.mkdir()
            (seed / dinov3.DINO_CLASSIFIER_FILENAME).write_bytes(b"classifier")
            (seed / dinov3.DINO_MODEL_MANIFEST_FILENAME).write_text(
                json.dumps(
                    {
                        "schema_version": 1,
                        "backend": "dinov3",
    ...
    def test_failed_install_preserves_previous_component(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            seed.mkdir()
            (seed / dinov3.DINO_CLASSIFIER_FILENAME).write_bytes(b"classifier")
            (seed / dinov3.DINO_MODEL_MANIFEST_FILENAME).write_text("{}", encoding="utf-8")

            with self.assertRaises(RuntimeError):
                dinov3.install_dinov3_component(
                    root=root,
                    cloud_client=_FakeDinoCloud(fail_on="source"),
                    seed_dir=seed,
                )
