...
def test_multi_dual_routes_all_four_signal_quadrants() -> None:
    config = MultiDualRejectionConfig(
        cosine_threshold=0.8,
        squared_distance_threshold=0.4,
    )

    assert config.route(cosine_score=0.9, squared_distance=0.1) == "update_prototype"
    assert config.route(cosine_score=0.9, squared_distance=0.9) == "candidate"
    assert config.route(cosine_score=0.2, squared_distance=0.1) == "candidate"
    assert config.route(cosine_score=0.2, squared_distance=0.9) == "new_mode_candidate"
