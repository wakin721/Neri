...
def _resolve_model_path(model_path: str | None) -> Path:
    default_model_dir = Path(resource_path("res/model"))
    candidate = Path(model_path).expanduser() if model_path else default_model_dir
    if not candidate.is_absolute() and not candidate.exists():
        candidate = Path(resource_path(str(candidate)))
