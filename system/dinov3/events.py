...
def camera_id_for_path(path:str|Path,root:str|Path|None=None)->str:
    ...
    if root is not None:
        try:
            relative=source.resolve().relative_to(Path(root).expanduser().resolve())
            return relative.parts[0] if len(relative.parts)>1 else (Path(root).resolve().name or "root")
        except (OSError,ValueError): pass
    return source.parent.resolve().name or "root"
