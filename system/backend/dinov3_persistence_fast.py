    ...
    with example_frame_cache():
        for context in prepared:
            ...
            if registry is None:
                continue
            if (
                runtime_observation.source == "checkpoint"
                and runtime_observation.accepted
            ):
                continue
            started = time.perf_counter()
            try:
                ...
                if runtime_observation.registry_id is not None:
                    registry.record_observation(
                        int(runtime_observation.registry_id),
