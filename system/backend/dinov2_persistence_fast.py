"""Batched runtime persistence for DINOv2 observations."""
...
from system.dinov2.feedback import FeedbackObservation
    ...
    for observation in observations:
        try:
            ...
            if (
                ...
                logger.warning(
                    "Ignoring out-of-range DINOv2 observation index: %s",
                    index,
                )
                continue
            ...
            logger.warning("Failed to prepare DINOv2 observation: %s", exc)
    ...
    if feedback is not None and feedback_observations:
        ...
        if callable(persist_many):
            try:
                ...
                logger.warning(
                    "Batch DINOv2 observation persistence failed; falling back: %s",
                    exc,
                )
                for observation in feedback_observations:
                    try:
                        ...
                        logger.warning(
                            "Failed to persist DINOv2 observation %s: %s",
                            observation.id,
                            row_exc,
                        )
        else:
        ...
        for context in prepared:
            ...
            if feedback is not None and feedback_observation is not None:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to persist DINOv2 feedback example %s: %s",
                        feedback_observation.id,
                        exc,
                    )
                example_elapsed += time.perf_counter() - started

            if registry is None or runtime_observation.accepted:
                continue
            started = time.perf_counter()
            try:
                registry_kwargs = {
                ...
                # never mutate an existing learned prototype. Only explicit
                # human feedback owns that authority.
                ...
                logger.warning("Failed to persist DINOv2 Registry evidence: %s", exc)
            registry_elapsed += time.perf_counter() - started

    logger.info(
        (
            "DINOv2 persistence timing: observations=%d prepare=%.3fs db=%.3fs "
